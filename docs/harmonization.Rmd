...
$$
   ...
   * Compute a loss value (with optional variance normalisation).
...
Locally around the optimum ((s_0, t_0)), the loss function is approximated by a general quadratic form:
...
The ellipse describes the joint uncertainty in ((s,t)).
However, we often want **individual uncertainties** for each parameter.
...
To obtain the **uncertainty in scale**, we project the ellipse onto the horizontal axis:
